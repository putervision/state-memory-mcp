...
- Hardened cross-server correlation and verification with the Pentad autonomous suite.
...
- **MCP Registry Schema Compliance**: Updated `server.json` fields (`registryType`, `identifier`, `source`, and repository URL) to pass official MCP Registry validator (`mcp-publisher validate`).
- **Version Bump**: Bumped version to `0.9.31` across package.json, server.json, manifest.json, documentation, issue templates, and test fixtures.
...
### Audit Updates, Version Bump & Security Alignment
- **Full Project Audit Update**: Reviewed and updated `PLAN-8-1-26.md` for `v0.9.1`, accurately tracking 58 test files (202 tests) and performance benchmark SLAs.
- **Dynamic Webhook Versioning**: Replaced hardcoded version string in `src/engine/notifications.ts` with `VERSION` constant.
- **Application Version Bump**: Bumped version to `0.9.1` across `package.json`, `package-lock.json`, `server.json`, documentation, and manifest artifacts.
...
- **Direct Binary Execution Pattern**: Standardized documentation, CLI help text, website guides, and manifests to use local binary execution (`state-memory-mcp run`).
- **Production Documentation Polish**: Hardened installation guides and project version strings across API reference docs and package manifests.
