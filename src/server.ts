  ...
  const tools = [
    {
      ...
      },
      {
        ...
        description: 'Query state graph using natural language free-text ("what is blocking auth?", "decisions led here").',
        inputSchema: {
          type: 'object',
          properties: {
            project: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            agent_role: { type: 'string', description: 'Optional role of posting agent (e.g. coder, reviewer, planner).' },
            ...
            ttl_seconds: { type: 'number', description: 'Optional time-to-live in seconds before message expires.' },
          },
      ...
      },
      {
        ...
        description: 'Atomically create a feature plan, optional milestone, subtasks, and dependency edges in a single turn.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            ...
            milestone_title: { type: 'string', description: 'Optional milestone title to group subtasks under.' },
            subtasks: {
              type: 'array',
              items: {
                type: 'object',
                properties: {
                  ...
                  depends_on_index: { type: 'number', description: 'Optional 0-based index of preceding subtask this task depends on.' },
                },
                required: ['title'],
              },
      ...
      },
      {
        ...
        description: 'Analyze an agent session event log and generate a post-mortem observation and report artifact.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      },
      {
        ...
        description: 'Reconstruct historical state memory (nodes and edges) as it existed at a specific ISO timestamp.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            timestamp: { type: 'string', description: 'Target ISO timestamp (e.g. 2026-07-28T12:00:00Z).' },
          },
          required: ['timestamp'],
        },
      },
      {
        ...
        description: 'Roll back state graph memory to a historical point in time, removing subsequent nodes and edges.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      },
      {
        ...
        description: 'Validate cross-memory references to external file paths and code symbols, optionally auto-healing broken links.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            auto_heal: { type: 'boolean', description: 'Optional flag to flag broken nodes with warning metadata.' },
          },
        },
      },
      {
        name: 'velocity_analytics',
        description: 'Calculate velocity analytics, task completion rate, average cycle time, and daily throughput breakdown.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      {
        name: 'burndown_chart',
        description: 'Generate burndown chart time-series data, remaining scope, and estimated completion date.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      {
        name: 'export_issues',
        description: 'Export tasks and blockers as external issue tracker JSON payloads (GitHub Issues / Jira).',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            format: { type: 'string', enum: ['github', 'jira', 'generic'], description: 'Target issue tracker format (default github).' },
          },
        },
      },
      {
        name: 'import_issues',
        description: 'Import external issue tracker issues (GitHub Issues / Jira) into state graph memory.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      },
      {
        ...
        description: 'Analyze state memory nodes created or modified on the current git branch vs a target branch.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            target_branch: { type: 'string', description: 'Target git branch to compare against (default main).' },
          },
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            strategy: { type: 'string', enum: ['auto_accept', 'flag_conflicts'], description: 'Conflict resolution strategy (default flag_conflicts).' },
          },
      ...
      {
        name: 'compact_graph',
        description: 'Rebuild database indexes, prune orphaned edges, and reclaim unused SQLite disk storage.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            prune_orphaned_edges: { type: 'boolean', description: 'Optional flag to prune orphaned edges (default false).' },
          },
        },
      },
      {
      ...
      {
        name: 'doctor_report',
        description: 'Run system health diagnostics, check schema integrity, WAL status, orphaned edge counts, and generate recommendations.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      },
      {
        ...
        description: 'Observe recent state graph mutations and event log entries since a timestamp or session.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            since_timestamp: { type: 'string', description: 'Optional ISO timestamp to filter events from.' },
      ...
      },
      {
        ...
        description: 'Link a task or artifact to a visual memory state ID via renders_state, blocked_by_visual_state, or verifies_visual_state edge.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
            ...
            visual_description: { type: 'string', description: 'Optional description of the visual state.' },
      ...
      },
      {
        ...
        description: 'Export unified, interleaved event and visual observation trajectories correlated by session ID for agent training.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
      ...
      },
      {
        ...
        description: 'Get combined dual-memory metrics: token savings, UI task verification ratio, and visual blocker health.',
        inputSchema: {
          type: 'object',
          properties: {
            project: { type: 'string', description: 'Optional project identifier.' },
    ...
    return {
      tools: tools.map(t => {
        ...
        const isReadOnly = [
          ...
          'get_spec_compliance', 'export_spec', 'export_joint_trajectories', 'get_synergy_metrics'
        ].includes(t.name);
